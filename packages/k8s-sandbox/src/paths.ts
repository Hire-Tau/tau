 ...
 *
 * Mirrors the entrypoint's `WS=${TAU_DEVBOX_DIR:-${WORKSPACE_PATH:-/workspace}}`.
 ...
 * TAU_DEVBOX_DIR (e.g. /private). The executor MUST resolve the same dir the
...
export function getDevboxDir(): string {
  return process.env.TAU_DEVBOX_DIR || getWorkspace()
}
...
function getAllowedPrefixes(): string[] {
  ...
  // path that is NOT a static ALLOWED_PREFIX. box-manager bakes TAU_BOX_HOME=<box
  // HOME> so the server permits writes under exactly that one box's home. k8s pods
  // NEVER set TAU_BOX_HOME, so this widens nothing there; it is scoped per-box.
  const boxHome = process.env.TAU_BOX_HOME
 ...
 *
 * k8s pods and docker sandboxes NEVER set `TAU_BOX_HOME`, so this is the
 ...
 *   /private            → $TAU_BOX_HOME/.private   (agent private scratch/key)
 *   /workspace[/<sq>]   → $TAU_BOX_HOME/workspace  (squad working tree)
 *   /memory[/<sq>]      → $TAU_BOX_HOME/memory     (memory replica)
 ...
 * squad id arrives as `TAU_SQUAD_ID`; the namespaced rules are matched BEFORE the
...
export function rebaseLogicalRoot(p: string): string {
  const home = process.env.TAU_BOX_HOME
  if (!home) return p
...
function boxRebaseRules(home: string): Array<{ logical: string; physical: string }> {
  const squadId = process.env.TAU_SQUAD_ID
 ...
 *
 * On a VM box (`TAU_BOX_HOME` set) an absolute LOGICAL root is first rebased onto
