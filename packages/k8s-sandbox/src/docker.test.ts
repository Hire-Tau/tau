...
const originalRole = process.env.TAU_SANDBOX_ROLE
...
afterEach(() => {
  restore('TAU_SANDBOX_ROLE', originalRole)
  restore('TAU_BOX_HOME', originalBoxHome)
...
describe('waitForDockerReady', () => {
  it('is a no-op on agent boxes (never spawns dockerd, resolves false fast)', async () => {
    process.env.TAU_SANDBOX_ROLE = 'agent'
...
describe('dockerRuntimeMode', () => {
  it("is 'sysbox' when TAU_BOX_HOME is unset (k8s/local — path byte-identical to pre-box)", () => {
    delete process.env.TAU_BOX_HOME
    expect(dockerRuntimeMode()).toBe('sysbox')
  })

  it("is 'rootless-box' when TAU_BOX_HOME is set (vm box)", () => {
