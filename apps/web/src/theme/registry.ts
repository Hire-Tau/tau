...
export const HIGH_CONTRAST_THEME_ID = 'high-contrast'
...
export const BUILT_IN_THEMES: readonly WebThemeDefinition[] = [
  ...
  // Constant light scheme: stark ink/paper, strong outlines and dark code panels. Always listed last (see
  // highContrastLast).
  { id: HIGH_CONTRAST_THEME_ID, label: 'High contrast', kind: 'unified', variantClass: { constant: null } },
]

/**
 * High contrast is an accessibility choice rather than a palette, so every theme picker lists it after all other
 * themes, the user's saved presets included.
 */
export function highContrastLast<T extends { kind: string; id: string }>(options: readonly T[]): T[] {
  const isHighContrast = (option: T) => option.kind === 'builtin' && option.id === HIGH_CONTRAST_THEME_ID
  return [...options.filter((option) => !isHighContrast(option)), ...options.filter(isHighContrast)]
}
