...
describe('createExeApi.createVm', () => {
  ...
  it('includes `--image <image>` in the `new` argv when an image is provided', async () => {
    ...
    await api.createVm({ name: 'vm-img', image: 'ghcr.io/ficushq/ficus-machine:latest' })

    expect(calls).toEqual([['new', '--name', 'vm-img', '--image', 'ghcr.io/ficushq/ficus-machine:latest', '--json']])
  })
