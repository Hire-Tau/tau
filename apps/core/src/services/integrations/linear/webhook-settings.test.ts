...
import {
  ...
  LEGACY_LINEAR_WEBHOOK_SECRET_KEY,
  migrateLinearWebhookSettings,
...
test('settings encrypt signing material, return only status and preserve public base paths', async () => {
  ...
  expect(await store.list()).not.toContainEqual(expect.objectContaining({ key: LINEAR_WEBHOOK_SETTINGS_KEY }))
  await configureLinearWebhook({ secret: null }, store, 'user:test')
  expect(resolveLinearWebhookSecret(store)).toBeNull()
  expect(getLinearWebhookSettings(store).configured).toBe(false)
})

test('invalid settings cannot change an existing secret', async () => {
  await configureLinearWebhook({ secret: 'original' }, store, 'user:test')
  for (const input of [
    {},
    { secret: '' },
    { secret: ' ' },
    ...
    await expect(configureLinearWebhook(input, store, 'user:test')).rejects.toThrow()
    expect(resolveLinearWebhookSecret(store)).toBe('original')
  }
})

test('startup migrates legacy database settings once and removes the old row', async () => {
  await store.set(LEGACY_LINEAR_WEBHOOK_SECRET_KEY, 'legacy-db-secret')
  const upgraded = new SecretStore()
  await upgraded.initialize()
  expect(resolveLinearWebhookSecret(upgraded)).toBe('legacy-db-secret')
  expect(await db.select().from(secrets).where(eq(secrets.key, LEGACY_LINEAR_WEBHOOK_SECRET_KEY))).toHaveLength(0)
})

test('legacy environment import cannot restore a disabled or rotated webhook on restart', async () => {
  process.env.LINEAR_WEBHOOK_SECRET = 'legacy-env-secret'
  const upgraded = new SecretStore()
  await upgraded.initialize()
  expect(resolveLinearWebhookSecret(upgraded)).toBe('legacy-env-secret')
  for (const secret of ['rotated-secret', null]) {
    await configureLinearWebhook({ secret }, upgraded, 'user:test')
    const restarted = new SecretStore()
    await restarted.initialize()
    expect(resolveLinearWebhookSecret(restarted)).toBe(secret)
  }
})

test('migration uses current durable settings even when its local cache predates a rotation', async () => {
  await store.set(LEGACY_LINEAR_WEBHOOK_SECRET_KEY, 'old-secret')
  const writer = new SecretStore()
  await writer.initialize()
  await configureLinearWebhook({ secret: 'new-secret' }, writer, 'user:test')
  await migrateLinearWebhookSettings(store)
  expect(resolveLinearWebhookSecret(store)).toBe('new-secret')
})
