...
describe('startTestDb when a database is required (CI and the package runner)', () => {
  ...
  test('the fallback is allowed only outside CI and without TAU_TEST_REQUIRE_DB', () => {
    ...
    expect(testDbFallbackAllowed({ TAU_TEST_REQUIRE_DB: '1' })).toBe(false)
    expect(testDbFallbackAllowed({ TAU_TEST_REQUIRE_DB: '0' })).toBe(true)
  })
})
...
describe('the unavailable-database stand-in', () => {
  ...
  test('the message and banner name the cause and the recovery', () => {
    ...
    expect(banner).toContain('TAU_TEST_REQUIRE_DB=1')
  })
