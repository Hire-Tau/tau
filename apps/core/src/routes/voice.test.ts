...
describe('GET /api/voice/status', () => {
  test('reports enabled when an OpenAI key is configured', async () => {
    ...
    expect(await res.json()).toEqual({ enabled: true, realtimeEnabled: true })
  })
  ...
  test('reports disabled when no OpenAI key is configured', async () => {
    ...
    expect(res.status).toBe(200)
    expect(await res.json()).toEqual({ enabled: false, realtimeEnabled: false })
  })
  ...
  test('treats an empty or whitespace-only key as absent', async () => {
    ...
    setOpenAIKey('')
    expect(await (await getStatus(admin.token)).json()).toEqual({ enabled: false, realtimeEnabled: false })
  ...
  test('never leaks the key, its length, or any prefix of it', async () => {
    ...
    expect(body).toBe('{"enabled":true,"realtimeEnabled":true}')
    expect(body).not.toContain('sk-')
    expect(body).not.toContain('secret')
    expect(Object.keys(JSON.parse(body))).toEqual(['enabled', 'realtimeEnabled'])
  })
...
test('realtime can be disabled without disabling transcription, and sessions are rejected', async () => {
  ...
  try {
    ...
    expect(await (await getStatus(admin.token)).json()).toEqual({ enabled: true, realtimeEnabled: false })
    const app = new Hono()
