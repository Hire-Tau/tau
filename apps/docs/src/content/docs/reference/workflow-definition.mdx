...
**Code hosting** (`completion.followChanges: true`) subscribes to linked PR and issue updates through the stream's code-hosting adapter, including PR comments, reviews, CI, and merges, plus issue comments, edits, and assignment changes. GitHub issues are attached with `github.repo` and `github.issue` in the work stream's metadata. Set `completion.changeEventsTo: { step: engineer }` to send the entire event bundle to an existing agent step. Omit it or use `delivery-owner` for automatic delivery ownership. Events are retained while the recipient cannot receive them; they do not start a future step. It is optional and does not itself grant repository access or authorize delivery.
