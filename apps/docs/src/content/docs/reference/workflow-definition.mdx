...
Save this as `brief-review.yaml` for `tau workflow create`. In the visual editor's complete-definition field, use only the contents beneath `definition`.
...
| `model`       | Optional model selection override. Use a selection supported by your instance's model configuration.                                                                              |
...
A participant is a named role in this flow. It directly selects an agent type and owns its model override and session policy; there is no separate profile to configure. Agent steps reference the participant ID.
...
`completion.followChanges: true` subscribes to review, CI, merge, and other change updates through the stream's code-hosting adapter. Set `completion.changeEventsTo: { step: engineer }` to send the entire event bundle to an existing agent step. Omit it or use `delivery-owner` for automatic delivery ownership. Events are retained while the recipient cannot receive them; they do not start a future step. It is optional and does not itself grant repository access or authorize delivery.
...
3. Write a preset file for reusable configuration or a source file for one-off work. Validate the source with `tau workflow resolve source.yaml --squad SQUAD_ID`. This checks resolution in the target squad without creating a stream or workers.
4. Publish only when requested: `tau workflow create preset.yaml`, or `tau workflow update PRESET_ID preset.yaml --revision REVISION_FROM_GET`. A revision conflict requires re-reading and reconciling changes.
5. Create requested work with `tau workstream create "Title" --squad SQUAD_ID --workflow PRESET_ID`, or `--flow source.yaml` for an inline/customized source.
