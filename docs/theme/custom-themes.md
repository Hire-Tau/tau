...
In Settings → Theme, **Edit custom theme** opens a device-local token editor. Importing or editing changes only its preview. **Apply custom theme** confirms the selection; **Reset to default** returns to Tau/light and removes the saved custom document. Selecting another built-in or appearance also clears custom overrides. Export downloads `tau-custom-theme.json`; no gallery, upload, server storage or account sync is involved.
...
A document defines **one concrete appearance**, not an automatic light/dark pair. Dual bases accept `light` or `dark`; High contrast requires `constant`. Missing tokens inherit the selected built-in, including its fractional syntax channels, terminal selection/scrollbar sentinels, supporting graph/chart/ANSI slots and intrinsic-opacity metadata. Brand tokens remain inactive.
...
- Any status override requires the **entire nine-role, seven-slot grid**, validated before inheritance. The picker initializes this set from the base when editing a status token and removes it as a set. Imports must supply it explicitly.
