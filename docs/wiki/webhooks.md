...
4. **Activity and delivery** — independent of each other: the fact projects into `squad_activity` under the `linear-issue` family (lane 71, kind `issue`) for every stream tracking the issue, whether or not any work stream or agent is listening; separately, the event is matched against work-stream and squad subscriptions and recorded in `integration_output_deliveries`. See [Linear integrations → Activity](linear-integrations.md#activity).
