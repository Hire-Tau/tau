...
`/device/start` accepts an optional `platform: 'cli' | 'desktop'` in its JSON body (default `cli`); any other value answers `400 invalid_platform`. The response echoes `platform` so a caller — Tau Desktop included — can detect whether the server understands desktop pairing before relying on it. A desktop-initiated grant defaults its device name to "Tau Desktop" when the caller sends none, the same way a CLI grant defaults to "Tau CLI".
